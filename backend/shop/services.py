...
# Qisqa TTL ataylab: kesh `equip_item()` da tozalanadi, lekin LocMemCache HAR BIR
# jarayonda alohida (settings.CACHES: Redis faqat REDIS_URL bo'lsa). Ya'ni bir ishchi
# jarayonda taqilgan bezak boshqasida hali eski holatda ko'rinishi mumkin. 30 daqiqada
# bu "sotib oldim, lekin o'zgarmadi" degan taassurot beradi; 2 daqiqa esa yuklamani
# ham kamaytiradi, ham noto'g'ri holat uzoq turmasligini kafolatlaydi.
EQUIPPED_CACHE_TTL = 120
...
def get_equipped_bulk(profile_ids):
    """{profile_id: {category: {...}}} — bir nechta profil uchun BITTA so'rovda.

    Reyting jadvalida 15-25 ta qator bo'ladi; har biri uchun `get_equipped()` chaqirish
    o'sha qadar alohida so'rov degani. Bu yerda hammasi bitta `profile_id__in` so'rovi
    bilan olinadi."""
    result = {pid: {} for pid in profile_ids}
    if not profile_ids:
        return result

    qs = (InventoryItem.objects
          .filter(profile_id__in=profile_ids, is_equipped=True,
                  item__category__in=ShopItem.EQUIPPABLE_CATEGORIES)
          .select_related('item'))
    for inv in qs:
        result[inv.profile_id][inv.item.category] = {
            'slug': inv.item.slug,
            'name': inv.item.name,
            'icon_name': inv.item.icon_name,
            'rarity': inv.item.rarity,
...
def display_avatar_url(profile_avatar_url, cosmetics):
    """Ekranda ko'rinadigan avatar: taqilgan avatar kosmetikasi ustun."""
    payload = (cosmetics or {}).get('avatar', {}).get('payload') or {}
    return payload.get('avatar_url') or profile_avatar_url
