...
# Next.js frontend origin(s) (see frontend/) — the JSON API under /api/ is JWT-authenticated
# (Authorization header, no cookies), so CORS_ALLOW_CREDENTIALS stays off; CORS only needs
# to let the browser read the response. Comma-separated in production via FRONTEND_ORIGINS.
CORS_ALLOWED_ORIGINS = [
    o.strip() for o in os.environ.get(
        'FRONTEND_ORIGINS', 'http://localhost:3000,http://localhost:3500',
    ).split(',') if o.strip()
]
...
# Next.js frontend manzili — Django endi faqat API, barcha sahifa havolalari
# (referral, Telegram deep link'dan keyingi sahifa) shu manzilga qaraydi.
...
# Channel the bot posts the daily "qiyin savol" to (numeric id, e.g. -100123456789, or
...
INSTALLED_APPS = [
    # Must load before django.contrib.staticfiles: its presence is what makes
    # `manage.py runserver` serve ASGI (and therefore WebSockets) instead of plain WSGI,
    # so Battle Arena's live PvP works locally with the same command as always.
    'daphne',
    'channels',
    'corsheaders',
    'rest_framework',
...
# Loyihaning o'z shablonlari YO'Q — butun UI Next.js (frontend/) tomonida.
# Bu blok faqat Django admin va DRF'ning o'z shablonlari uchun kerak (APP_DIRS).
TEMPLATES = [
    {
...
# Channels (WebSockets — Battle Arena live PvP matchmaking).
# Same REDIS_URL knob as CACHES above: with Redis, matchmaking/battle groups are shared
# across every ASGI worker process (required once there's more than one worker, e.g.
# WEB_CONCURRENCY > 1 in production). Without it, InMemoryChannelLayer keeps everything
# in a single process — correct for local `manage.py runserver`, but two players would
# only find each other if they hit the same worker, so it must not be used with >1 worker.
...
# Fail loud instead of silently degrading: a real (DEBUG=False) deployment without
# DATABASE_URL/REDIS_URL set would otherwise start up fine on SQLite + LocMemCache +
# InMemoryChannelLayer, "work" for a single visitor, and only break once real concurrent
# traffic hits it (locked-database errors, per-worker-inconsistent caches, Battle Arena
# matchmaking silently failing across workers — see the comments above). Catching the
# missing env var at boot is far cheaper than debugging that under a live traffic spike.
if not DEBUG:
    _missing = [name for name, val in (('DATABASE_URL', DATABASE_URL), ('REDIS_URL', REDIS_URL)) if not val]
    if _missing:
        from django.core.exceptions import ImproperlyConfigured
        raise ImproperlyConfigured(
            f"{', '.join(_missing)} must be set when DEBUG=False (production) — see "
            "deploy/DEPLOY_VPS.md. Running production on SQLite/in-memory cache silently "
            "breaks under real concurrent traffic instead of failing here at startup."
        )
...
# DRF API for the Next.js frontend (frontend/). JWT, not session auth — the whole point is
# to avoid the SameSite/CORS cookie problems a cross-origin SPA would hit with Django's
# normal session-cookie login (see accounts/api.py docstring for the full reasoning).
from datetime import timedelta

REST_FRAMEWORK = {
    'DEFAULT_AUTHENTICATION_CLASSES': (
        'accounts.jwt_auth.AuditAwareJWTAuthentication',
    ),
...
# CEFR Listening va boshqa audio/video fayllar uchun maksimal hajm (150MB)
