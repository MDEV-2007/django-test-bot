...
def get_referral_link(profile: Profile, request=None) -> str:
    """Ulashish uchun to'liq havola.

    Ro'yxatdan o'tish sahifasi endi Django'da emas, Next.js frontendida (`/register`).
    Shuning uchun manzil `reverse()` bilan emas, `FRONTEND_URL` (yoki `WEBAPP_URL`)
    asosida quriladi; sozlama bo'lmasa — joriy so'rov hosti ishlatiladi.
    """
    from django.conf import settings

    code = ensure_referral_code(profile)
    path = f'/register?ref={code}'
...
def apply_referral(new_profile: Profile, code: str) -> Profile | None:
    ...
    # O'qituvchi-orqali-sinf (Feature 1): agar taklif qilgan odam o'qituvchi bo'lsa,
    # o'quvchi avtomatik uning sinfiga bog'lanadi. Alohida "sinf kodi" o'ylab topilmadi —
    # o'qituvchi allaqachon bor referral kodini/Telegram deep-linkini tarqatadi, shuning
    # uchun o'qituvchi uchun qo'shimcha qadam yo'q.
    if referrer.role == 'teacher' or referrer.is_teacher:
        from teacher.models import TeacherStudent
        TeacherStudent.objects.get_or_create(student=new_profile, defaults={'teacher': referrer})
