...
def make_user(username='student', **profile_kwargs):
    ...
    # create_user() above already triggered the post_save signal that auto-creates a
    # Profile and caches it onto user.profile; ensure_profile_for_user() here just
    # fetches that same row via a fresh get_or_create() call, so it does NOT return
    # the same Python object. Applying profile_kwargs to *that* object and saving
    # updates the DB row correctly, but leaves user.profile pointing at the stale,
    # pre-kwargs object. Refresh the cache so callers reading user.profile see it.
