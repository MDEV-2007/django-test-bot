...
import {
  ...
  Zap, BarChart3, PieChart, ShieldCheck, ChevronRight, RefreshCw, Send, Phone,
...
const STAT_CARDS: { key: string; label: string; icon: typeof Users; tone: string }[] = [
  { key: 'users', label: 'Foydalanuvchilar', icon: Users, tone: 'text-primary' },
  ...
  { key: 'testsets', label: 'Testlar', icon: FileCheck2, tone: 'text-primary' },
  { key: 'lessons', label: 'Darslar', icon: BookOpen, tone: 'text-sky-400' },
  { key: 'games', label: "O'yinlar", icon: Gamepad2, tone: 'text-purple-400' },
  { key: 'attempts_today', label: 'Bugungi urinishlar', icon: Activity, tone: 'text-emerald-500' },
  { key: 'attempts_total', label: 'Jami topshirilgan', icon: TrendingUp, tone: 'text-emerald-500' },
  { key: 'pending_payments', label: "Kutayotgan to'lovlar", icon: CreditCard, tone: 'text-amber-500' },
  { key: 'total_revenue', label: 'Tushum', icon: Wallet, tone: 'text-amber-500' },
  { key: 'active_today', label: 'Bugun faol', icon: Radio, tone: 'text-emerald-500' },
...
const ACTION_TONE: Record<string, string> = {
  ...
  update: 'bg-blue-500/15 text-blue-600 dark:text-blue-400 border-blue-500/30',
  ...
  return (
    <PanelShell>
      <div className="space-y-6">
        ...
        {/* Sarlavha & Tezkor boshqaruv */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2.5">
              <BarChart3 className="size-6 text-primary" /> Boshqaruv Paneli &amp; Analitika
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Platformaning umumiy holati, o&apos;quvchilar faolligi va chuqurlashtirilgan ta&apos;limiy tahlili.
            </p>
          </div>
          <div className="flex items-center gap-2.5">
            ...
            <Link href="/panel/mocks">
              <Button size="sm" className="gap-1.5 text-xs h-9 font-semibold bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white shadow-xs">
                <Trophy className="size-3.5" /> Mock Hisobotlariga O&apos;tish
              </Button>
            </Link>
          </div>
        </div>

        {/* 1. ASOSIY STRATEGIK KO'RSATKICHLAR (EXECUTIVE METRICS) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          {/* A. Tugatish darajasi (Completion rate) */}
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-black text-foreground">
                  {loading ? <Skeleton className="h-7 w-20" /> : `${stats.completion_rate ?? 0}%`}
                </span>
          ...
          <Card className="border border-border/70 shadow-xs bg-gradient-to-br from-card to-card/60 relative overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-black text-foreground">
                  {loading ? <Skeleton className="h-7 w-20" /> : `${stats.avg_score ?? 0}%`}
                </span>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Barcha topshirilgan testlar bo&apos;yicha umumiy o&apos;rtacha
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border border-border/70 shadow-xs bg-gradient-to-br from-card to-card/60 relative overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-black text-foreground">
                  {loading ? <Skeleton className="h-7 w-20" /> : `${stats.pass_rate ?? 0}%`}
                </span>
          ...
          <Card className="border border-border/70 shadow-xs bg-gradient-to-br from-card to-card/60 relative overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-black text-foreground">
                  {loading ? <Skeleton className="h-7 w-20" /> : `${stats.tg_connected ?? 0}`}
                </span>
        ...
        {/* 2. TEZKOR MA'MURIY RAQAMLAR STRIP (12 TA KO'RSATKICH) */}
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-6 xl:grid-cols-6">
          {loading && Array.from({ length: 12 }).map((_, i) => (
            <Card key={`sk-${i}`} className="py-2.5">
          ...
          {!loading && data && STAT_CARDS.filter((c) => c.key in data.stats).map((c) => {
            const Icon = c.icon;
            return (
              <Card key={c.key} className="gap-0 py-2.5 transition-all hover:border-border-strong bg-card/50">
                <CardContent className="px-3">
                  <div className="flex items-center gap-1.5">
                    <Icon className={`size-3 shrink-0 ${c.tone}`} />
                    <p className="truncate text-[11px] text-muted-foreground font-medium">{c.label}</p>
                  </div>
                  <p className="mt-1 font-mono text-base font-bold text-foreground tabular-nums">{fmt(data.stats[c.key])}</p>
                </CardContent>
              </Card>
            );
          })}
        ...
        {/* 3. JONLI MOCK VA MOCK ANALITIKASI KARTASI */}
        {(upcomingMock || (mockAnalytics && mockAnalytics.total_attempts > 0)) && (
          <Card className="border border-amber-500/40 bg-gradient-to-r from-amber-500/10 via-card to-card shadow-xs overflow-hidden">
            <div className="p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              {/* Mock tezkor raqamlari */}
              {mockAnalytics && (
                <div className="flex flex-wrap items-center gap-3 shrink-0">
                  <div className="bg-background/80 border border-border/70 rounded-xl p-3 min-w-[110px] text-center">
                  ...
                  <div className="bg-background/80 border border-border/70 rounded-xl p-3 min-w-[110px] text-center">
                    <p className="text-[11px] text-muted-foreground font-medium">Oltin daraja (80%+)</p>
        ...
        {/* 4. IKKI USTUNLI CHUQUR TAHLIL: BALLAR TAQSIMOTI VA JAVOBLAR ANATOMIYASI */}
        <div className="grid gap-4 lg:grid-cols-2">
          
          {/* A. Ballar darajalari taqsimoti */}
          ...
          {/* B. Javoblar anatomiyasi (To'g'ri / Xato / Bo'sh) */}
          <Card className="border border-border/70 shadow-xs">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
        ...
        {/* 5. FANLAR CHUQUR DIAGNOSTIKASI JADVALI */}
        ...
        {/* 6. FAOL SOATLAR (PEAK HOURS) VA O'QUVCHILAR SADOQATI (RETENTION) */}
        <div className="grid gap-4 lg:grid-cols-2">
          
          {/* A. Kunning faol soatlari (Peak Hours) */}
          <Card className="border border-border/70 shadow-xs">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
          ...
          {/* B. O'quvchilar sadoqati (Retention & Loyalty) */}
          <Card className="border border-border/70 shadow-xs">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
        ...
        {/* 7. TOP 5 PESHQADAM O'QUVCHILAR VA O'SISH DINAMIKASI */}
        <div className="grid gap-4 lg:grid-cols-2">
          
          {/* A. Top 5 Yetakchi O'quvchilar */}
          <Card className="border border-border/70 shadow-xs">
                  ...
                  {topStudents.map((st) => (
                    <div key={st.rank} className="flex items-center justify-between py-2.5 first:pt-0 last:pb-0">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="font-semibold text-xs text-foreground truncate max-w-[170px]">{st.name}</p>
          ...
          {/* B. O'sish dinamikasi (Kunlik ro'yxatdan o'tish va test urinishlari) */}
          <Card className="border border-border/70 shadow-xs">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <TrendingUp className="size-4.5 text-emerald-500" />
                O&apos;sish Dinamikasi (Oxirgi 7 kun)
              </CardTitle>
        ...
        {/* 8. ENG QIYIN SAVOLLAR VA AUDIT JURNALI */}
        <div className="grid gap-4 lg:grid-cols-2">
          
          {/* A. Eng qiyin savollar (HTML tozalangan va Fan ko'rsatilgan) */}
          <Card className="border border-border/70 shadow-xs">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
              ...
              <CardDescription>O&apos;quvchilar eng ko&apos;p xato qilgan savollar tahlili</CardDescription>
            </CardHeader>
            <CardContent>
              {data?.hardest_questions.length === 0 ? (
                <p className="py-6 text-center text-xs text-muted-foreground">Hali yetarli javob yo&apos;q.</p>
              ) : (
                    ...
                    <div className="flex items-start justify-between gap-3 py-2.5">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 mb-1">
                          {q.subject_name && (
                            <Badge variant="outline" className="text-[10px] px-1.5 py-0 bg-muted font-semibold">
                              {q.subject_name}
                            </Badge>
                          )}
                        </div>
                        <p className="text-xs leading-snug text-foreground line-clamp-2">{q.text}</p>
                      </div>
                      <div className="shrink-0 text-right">
                        <p className={`font-mono text-xs font-bold ${q.rate < 40 ? 'text-red-500' : 'text-amber-500'}`}>
                          {q.rate}% to&apos;g&apos;ri
                        </p>
                        <p className="text-[10px] text-muted-foreground">{q.total} ta javob</p>
                      </div>
                    </div>
                  </div>
                ))
          ...
          <Card className="border border-border/70 shadow-xs">
            <CardHeader className="flex-row items-center justify-between pb-3 space-y-0">
              <div>
                ...
                <CardDescription>Audit jurnalidan oxirgi amallar</CardDescription>
              </div>
              <Link href="/panel/audit-log" className="flex items-center gap-1 text-xs font-medium text-primary hover:underline">
                Hammasi <ArrowRight className="size-3" />
              </Link>
            </CardHeader>
            <CardContent>
              {data?.recent_logs.length === 0 ? (
                <p className="py-6 text-center text-xs text-muted-foreground">Hali yozuv yo&apos;q.</p>
              ) : (
                data?.recent_logs.slice(0, 8).map((log, i) => (
                  <div key={log.id}>
                    {i > 0 && <Separator />}
                    <div className="flex items-center gap-2.5 py-2.5">
                      <Badge variant="outline" className={`shrink-0 text-[10px] ${ACTION_TONE[log.action] ?? ''}`}>
                        {ACTION_LABEL[log.action] ?? log.action}
                      </Badge>
                      <p className="min-w-0 flex-1 truncate text-xs text-foreground">{log.summary}</p>
